...
function Row(props) {
  ...
  const totalBasketPrice = row.products
    .map((item) => {
      const final_price = Number(item.price_stock) > 0 ? Number(item.price_stock) : Number(item.price);
      return item.count_product * final_price;
    })
  ...
  //   .map((item) => {
  //     const current_product = product.allProducts.filter((pr) => pr.id === item.productId)[0];
  //     const final_price =
  //       Number(current_product.price_stock) > 0
  //         ? Number(current_product.price_stock)
  //         : Number(current_product.price);
  //     return item.count_product * final_price;
  //   })
  //   .reduce((prev, current) => prev + current, 0)
  //   .toFixed(2);
                  ...
                  {row.products.map((productsRow) => (
                    <TableRow key={productsRow.id}>
                      ...
                      <TableCell align="center">
                        {Number(productsRow.price_stock) > 0
                          ? Number(productsRow.price_stock)
                          : Number(productsRow.price)}
                      </TableCell>
